...
/**
 * Dependency graph generator for decision rules. Adds the declarative content
 * of a rule to a dependency graph.
 */
		...
		for (MentalLiteral lit : subject.getCondition().getAllLiterals()) {
			...
			if (hasFilterAction && lit.isPositive()) {
				if (lit instanceof AGoalLiteral || lit instanceof GoalLiteral) {
					for (DatabaseFormula formula : lit.getFormula().toUpdate().getAddList()) {
						try {
		...
		try {
			for (Action<?> action : subject.getAction()) {
				...
				if (action instanceof AdoptAction) {
					...
					for (DatabaseFormula formula : adopt.getUpdate().getAddList()) {
				 ...
				 */
					...
					// There should be no delete list.
					for (DatabaseFormula formula : drop.getUpdate().getAddList()) {
					...
					for (DatabaseFormula formula : delete.getUpdate().getDeleteList()) {
						super.getGraph().add(formula, true, false);
					}
					...
					for (DatabaseFormula formula : insert.getUpdate().getAddList()) {
						super.getGraph().add(formula, true, false);
					}
				}
				// ASSUMES that Graph Generators are run after Validators.
					...
					for (MentalLiteral formula : userspec.getPrecondition().getAllLiterals()) {
