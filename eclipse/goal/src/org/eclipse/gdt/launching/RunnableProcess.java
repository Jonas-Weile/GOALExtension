...
public abstract class RunnableProcess extends Process implements Runnable {
	 ...
	 * @param lazyStart
	 */
	public RunnableProcess(final String label, final boolean lazyStart) {
		final String threadName = ((label != null && label.length() > 0) ? label : generateThreadName());
	...
	public int waitFor() throws InterruptedException {
		synchronized (this) {
			while (this.state != RunnableState.TERMINATED) {
				wait();
			}
		}
		return 0;
	...
	public int exitValue() {
		if (this.state != RunnableState.TERMINATED) {
	...
	public InputStream getInputStream() {
		return this.inputStreamDelegate;
	}
	...
	public InputStream getErrorStream() {
		return this.errorStreamDelegate;
	}
	...
	public void destroy() {
		...
		try {
			...
			if (this.thread != null && this.thread.isAlive()) {
				// FIXME : // when runnable loop for while(true) {} Thread is
				// never killed.
	 ...
	 * @param inputStream
	 */
	public void out(final InputStream inputStream) {
	 ...
	 * @param s
	 */
	public void err(final Throwable e) {
	 ...
	 * @param inputStream
	 */
	public void err(final InputStream inputStream) {
	...
	protected void start() {
		if (this.state != RunnableState.INITIAL) {
			throw new UnsupportedOperationException("Runnable Process is already started.");
		}
		this.thread.start();
	 ...
	 * @return
	 */
	protected Thread createThread(final Runnable runnable, final String threadName) {
	...
	private void terminate() {
		...
		// kill the thread of OutputStreamMonitor
		...
		synchronized (RunnableProcess.this) {
			...
			RunnableProcess.this.notifyAll();
		}
	}

	protected boolean isTerminated() {
		return this.state == RunnableState.TERMINATED;
	}
