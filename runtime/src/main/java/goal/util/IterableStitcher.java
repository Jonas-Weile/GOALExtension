...
public class IterableStitcher<T> implements Iterable<T> {
	/**
	 * The iterable whose iterator iterates over the collections of values that this
	 * stitcher stitches.
	 */
	private final Iterable<? extends Iterable<? extends T>> baseIterable;

	/**
	 * Creates a new {@link IterableStitcher}, which will stitch the given iterable
	 * of iterables.
	 *
	 * @param baseIterable
	 *            The iteration of iterables to stitch together.
	 */
	public IterableStitcher(Iterable<? extends Iterable<? extends T>> baseIterable) {
	...
	/**
	 * Iterator for the {@link IterableStitcher}. Takes one Iterable and iterates
	 * over its values. Once the iterable's iterator is exhausted, the next iterable
	 * is taken. This repeats until all iterables have been iterated over.
	 */
	private class StitchIterator implements Iterator<T> {
		...
		/**
		 * The iterator to use for the next value in {@link #next()}, as well as for
		 * {@link #hasNext()}
		 */
		...
		/**
		 * Finds the next iterator in {@link #innerIterator} that can provide a value
		 * for {@link #next()}. That iterator is stored in {@link #currentIterator}.
		 */
