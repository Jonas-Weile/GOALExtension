...
public abstract class IterableFilter<T, U> implements Iterable<U> {
	...
	/**
	 * Checks if the given value should be part of the iterated collection. If so,
	 * the value should be converted and returned. If not, {@code null} is returned.
	 *
	 * @param value
	...
	/**
	 * The iterator for a filtered Iterable. Will find the next filtered value from
	 * the inner iterator after each next() call, as well as after construction, to
	 * prevent requiring a call to {@link #hasNext()} every time (although that is
	...
	private class FilteredIterator implements Iterator<U> {
		...
		/**
		 * Finds the next value in the iterator for which {@link IterableFilter#select}
